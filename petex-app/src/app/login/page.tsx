...
export default function LoginPage() {
  ...
  const BUILD_SUMMARY =
    'Login actualizado: nuevo diseño, acceso demo y validaciones activas';
  const showBuildSummary = buildContext !== 'production';
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-50 to-slate-100 flex items-center justify-center p-4">
      <Card className="w-full max-w-md p-6 sm:p-8">
        ...
        {showBuildSummary ? (
          <div className="mt-4 rounded-lg bg-slate-100 px-3 py-2 text-[11px] text-slate-600 flex items-center gap-2">
            <Info className="h-3.5 w-3.5 text-slate-500" />
            <span>{BUILD_SUMMARY}</span>
          </div>
        ) : null}
